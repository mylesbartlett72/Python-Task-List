    ...
    with dpg.window(label=f"Create a Task in {list_name}") as create_task_window:
        ...
        with dpg.table(header_row=False):
            ...
            with dpg.table_row():
                ...
                dpg.add_button(
                    label="Cancel", callback=lambda: dpg.delete_item(create_task_window)
                )
    ...
    with dpg.window(
        ...
        with dpg.table(header_row=False):
            ...
            with dpg.table_row():
                ...
                dpg.add_button(
                    label="Cancel", callback=lambda: dpg.delete_item(edit_task_window)
                )
            with dpg.table_row():
                if col == "To Do":
                    dpg.add_spacer()
    ...
    if not finished[col]:
        ...
        if task == None:
            element_uuid = dpg.add_button(
                label="Add Task",
                callback=lambda x, y: create_new_task_window(x, tasks, primary_window),
                tag=col,
            )
            finished[col] = True
    ...
    with dpg.window() as no_file_window:
        ...
        with dpg.table(header_row=False):
            ...
            with dpg.table_row():
                dpg.add_button(
                    label="Yes",
                    callback=lambda: no_file_window_btn_callback(True, no_file_window),
                )
                dpg.add_button(
                    label="No",
                    callback=lambda: no_file_window_btn_callback(False, no_file_window),
                )
